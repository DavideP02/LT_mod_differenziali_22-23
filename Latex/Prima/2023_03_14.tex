...
Ora, interpretando $u_{(0,x)}(t)$ come funzione di due variabili $ t $ e $ x $, analizziamone le derivate parziali:
 \begin{align*}
    \partial_t u_{(0,x)}(t)&=\dpd{}{t} u_{(0,x)}(t) = \cdots = t\,u^{2}_{(0,x)}(t)\\[3ex]
    \partial_x u_{(0,x)}(t)&=\dpd{}{x} u_{(0,x)}(t) = \dpd{}{x}\left(\frac{2x}{2-t^{2}x}\right)\\[2ex] 
...
\]La funzione $ v(t) $ risolve un'equazione \emph{lineare} che si chiama \emph{equazione alle variazioni} dove \[
    g(t)=\dpd{}{u}\left[f(t,u)\right]_{u=u_{(0,x_0)}(t)}.
\]
...
\paragrafo{Generalizzazione}{%
    ...
    \begin{align*}
        v'(t) =\dod{}{t} v(t) &= \partial_u f\left(t, u_{(t_0,x)}(t)\right) \cdot \partial_x u_{(t_0,x)}(t) 
