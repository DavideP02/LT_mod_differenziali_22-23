...
\dimostrazione{dlkjanbflkasjdbfnlakjnflkasjdnlkasdjnflaksdfjnlkjnlkjn}{
    \begin{itemize}
        ...
        \item[($\impliedby$)] \textit{Hp}: $ \varphi $ è soluzione di (PC)\textsubscript{$\varphi$} e $ f_1\left(x,\varphi(x)\right) \neq 0$ in un opportuno intorno di $ p_1 $, $ I_{p_1}  $ (poiché $ f $ è $ C^{1} $). 
        
        Allora, $ \forall\, x \in I_{p_1}  $ sia\[
            \omega(x)\coloneqq \int_{p_1}^{x}\frac{\dif\xi}{f_1\left(\xi, \varphi(\xi)\right)}.
...
\esempio{
    ...
    \begin{enumerate}
        ...
        \]che è a variabili separabili: \begin{align*}
            ...
            \int \varphi^{2}(x)\,\varphi'(x)\dif x & = \int -x^{2}\dif x \leadsto \int \varphi^2\dif \varphi = -\int x^{2}\dif x\\[2ex]
...
\begin{enumerate}
    ...
    Inoltre $ x'=y^{2}>0 $ lungo le orbite. Dunque il diagramma di fase è così composto: 
    \begin{figure}[H]
        \begin{center}
            \begin{tikzpicture}
                \draw [-Latex] (-3,0) -- (3,0);
