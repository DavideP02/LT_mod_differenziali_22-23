...
\paragrafo{Ipotesi}{%
    \begin{itemize}
        ...
        \item $ \bm{p} \in \Omega \colon \bm{f}(\bm{p})=\bm{0} $ (equilibrio).
        \item $ V\colon B_{r}(\bm{p})\longrightarrow \R  $, con $ r>0 $ e $ B_{r}(\bm{p}) \subseteq \Omega  $, $ V \in C^{1}( B_{r}(\bm{p})) $.
    \end{itemize}
...
\paragrafo{Monotonia di $ V $}{%
    ...
    \begin{itemize}
        ...
        \item Ne calcoliamo la monotonia in $ t $, definendo: \[
            \dot{V}\left(\bm{x}(t)\right)\coloneqq\od{}{t}V\left(\bm{x}(t)\right) 
        \]Dunque: \begin{align*}
            \dot{V}\left(\bm{x}(t)\right) &= \scalare{\nabla V\left(\bm{x}(t)\right)}{\bm{x}'(t)}= \scalare{\nabla V\left(\bm{x}(t)\right)}{\bm{f}\left(\bm{x}(t)\right)}
    ...
    Poiché sia $ V $ che $ \bm{f} $ sono di classe $ C^{1} \leadsto \nabla V $ è continua $\leadsto \dot{V} $ è continua. 

    $\dot{V}$ è detta la \emph{derivata totale di $ V $ rispetto al campo $ \bm{f} $.}
}{}{}
...
\dimostrazione{dididicoijcoijdoijcoijcoijsoij}{
    Dimostriamo solo la prima parte. La tesi è la stabilità di $ \bm{p} $, ovvero: $ \forall\, \varepsilon>0 $, $ \exists\, \delta = \delta( \varepsilon)>0 $ tale che, se $ \bm{q} \in B_{\delta}(\bm{p}) $, la soluzione $ \bm{\psi}_{\bm{q}}(t) $ del sistema \[
        ...
        \]soddisfi: \begin{itemize}
            \item $ T_{\max}= + \infty  $;
            \item $ \exists\, T\ge 0 \colon \forall\, t > T $ \[
                \bm{\psi}_{\bm{q}}(t) \in B_{ \varepsilon}(\bm{p}) 
            \]
        \end{itemize}
        
        Fisso $ \varepsilon< r $, in modo che $ V $ sia definita su $ B_{ \varepsilon}(\bm{p})  $, e definisco \[
            m_{ \varepsilon} = \min_{x \in \partial B_{ \varepsilon}(\bm{p}) } V(\bm{x})>0  
        \]poiché $ V $ è definita positiva. 

        Avendo $ V(\bm{p})=0 $ e $ V $ continua $\implies$ $ \exists\, \delta \in (0, \varepsilon) \colon V(\bm{x})< \frac{1}{2} m_{ \varepsilon}  $, $ \forall\, \bm{x} \in B_{ \delta}(\bm{p})  $

        Dimostro che questo $ \delta $ soddisfa la condizione di stabilità.

        Per assurdo, supponiamo che $ \exists\, \bm{q} \in B_{\delta}(\bm{p}) $ tale che la soluzione $ \bm{\psi}_{\bm{q}}(t)$ di \[
            \begin{cases}
                \bm{x}'=\bm{f}(\bm{x})\\ 
                \bm{x}(0)=\bm{q}
            \end{cases}
        ...
        Per ipotesi $ \dot{V}(\bm{x}) $ è semidefinita negativa, cioè $ V $ decresce lungo le soluzioni: \[
            m_{ \varepsilon} \le V\left(\bm{\psi}_{\bm{q}}(\tau)\right) \le V(\bm{q}) = V\left(\bm{\psi}_{\bm{q}}(0)\right) < \frac{m_{ \varepsilon} }{2}
        \]Assurdo!$ \qed$ 
}
\paragrafo{Corollario - Applicazione ai sistemi $ \ddot{\bm{x}}=-\nabla U (\bm{x})$}{
    Sia\\ $U: \Omega \subseteq \R^{n}\longrightarrow \R $, $ \Omega $ aperto e $ \bm{p} \in  \Omega$ 
    \begin{itemize}
        \item $ U \in C^{1} (\Omega)$;
