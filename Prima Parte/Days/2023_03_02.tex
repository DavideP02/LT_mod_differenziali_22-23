...
\paragrafo{Orbite non singolari}{%
    ...
    Se $ \bm{f}(\bm{p})\neq \bm{0} $, allora almeno una delle sue componenti è non nulla.\\
    Considerata $ f_{1}(\bm{p})\neq 0 \leadsto$ il vettore tangente all'orbita per $ \bm{p} $ non è verticale. 
    
    Sia allora il sistema: \[
        \begin{cases}
            x'=f_{1}(x,y)\\ 
            y' = f_{2}(x,y)\\ 
            \left(x(0),y(0)\right)=\bm{p}
        \end{cases}
    \]e $\bm{u} $ una soluzione massimale, tale che $ \bm{u}(0)=\bm{p} $. \\
    Sapendo che $ \bm{u}'(0) $ non è un vettore verticale possiamo esprimere localmente la seconda componente dell'orbita per $ \bm{p} $ in funzione della prima. Ovvero:\[
    ...
    \]e questa funzione descrive l'orbita \[
        \begin{cases}
            x=u_1(t)\\ 
            \begin{aligned}
                y=u_2(t)&= \varphi(x) &= \varphi\left(u_1(t)\right)
            \end{aligned}
        \end{cases}
    \]Si ha che \[
    ...
    Stiamo dicendo che se $ f_1(p)\neq 0 $, l'orbita che passa per $ \bm{p} $ è rappresentabile mediante una funzione $ y=\varphi(x) $ che soddisfa \[
        \begin{cases}
...
\dimostrazione{dlkjanbflkasjdbfnlakjnflkasjdnlkasdjnflaksdfjnlkjnlkjn}{
    \begin{itemize}
        ...
        \item[($\impliedby$)] \textit{Hp}: $ \varphi $ è soluzione di (PC)\textsubscript{$\varphi$}. Per ipotesi sappiamo che $ f_1\left(x,\varphi(x)\right) \neq 0$ in un opportuno intorno di $ p_1 $, $ I_{p_1}  $ (poiché $ f $ è $ C^{1} $). 
        
        Allora, $ \forall\, x \in I_{p_1}  $ sia\[
            \omega(x)\coloneqq \int_{p_1}^{x}\frac{d\,\xi}{f_1\left(\xi, \varphi(\xi)\right)}.
        ...
        $\implies$ $ \omega $ è invertibile. \[
             \begin{aligned}
                \omega: I_{p_1}\to I \ni 0 \quad\leadsto\quad\omega^{-1}= v\colon I&\longrightarrow I_{p_1}\\ 
                v(0) &= p_1\\ 
...
\esempio{
    ...
    \begin{enumerate}
        ...
        $\implies$ vi è un solo punto di equilibrio: $ \bm{0} $
        ...
        Il vettore tangente ad una soluzione in $ (x,y) $ è $ \bm{f}(x,y) $, dunque: \begin{itemize}
            \item \textit{Tangente verticale}: $ x'=0 $ $ \iff $ $ y^{2} =0$ 
            
            $\implies$ l'asse $ x $ viene intersecato verticalmente dalle orbite;
            \item \textit{Tangente orizzontale}: $ y'=0 $ $ \iff $ $ x^{2}=0 $ 
        ...
        Consideriamo i punti in cui $ f_1(x,y)\neq 0 $, ovvero i punti con $ y\neq 0 $. Cerchiamo l'equazione delle orbite dei punti che non sono sull'asse $ x $. \[
            \begin{cases}
                \varphi'(x)=-\cfrac{x^{2}}{\varphi^{2}(x)}\\
                \varphi(p_1)=p_2\tag*{(PC)\textsubscript{$\varphi$}}
            \end{cases}
        \]che è a variabili separabili: \begin{align*}
            \varphi^{2}(x)\,\varphi'(x) & = -x^{2}\\[2ex] 
            \int \varphi^{2}(x)\,\varphi'(x)\,dx & = \int -x^{2}\,dx \leadsto \int \varphi^2\,d\varphi = -\int x^{2}\,dx\\[2ex]
            \leadsto\frac{1}{3} \varphi^{3}(x) &= -\frac{1}{3}x^{3} + c
        \end{align*}
...
\begin{enumerate}
    ...
    Inoltre $ x'=y^{2}>0 $ lungo le orbite. Dunque il diagramma di fase è quello illustrato in figura \ref{fig:eqwsdcs}. 
    \begin{figure}[H]
        \begin{center}
            \begin{tikzpicture}
                \draw [-Latex] (-3,0) -- (3,0);
