...
\paragrafo{Domanda}{%
    Ci chiediamo ora se la funzione $ \bm{u}_{(t_0,\bm{x})}(t) $ ha regolarità maggiore rispetto a $ \bm{x} $? Se sì, come si comporta la funzione $ \partial_x\bm{u}_{(t,\bm{x})}(t) $?
}{}{}
...
\]$u_{(0,x)}(t)$ la interpretiamo come funzione di due variabili $ t $ e $ x $. \begin{align*}
...
\paragrafo{Definizione del flusso}{%
    ...
    Considero ora l'insieme \[
        E=\{(t,\bm{x}) \in\Omega \colon t \in I(\bm{x}), \bm{x} \in \Omega_{0}\}. 
    \]
...
\paragrafo{Sistemi autonomi}{%
    ...
    Se $ I(\bm{x}) $ è l'intervallo massimale per la soluzione con $ \bm{u}_{(0,\bm{x})} $ ed $ E $ è l'insieme: \[
