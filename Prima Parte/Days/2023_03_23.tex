...
\teorema{didididiocoijscjkdkjncjdjdkjncjd}{
    Considero il sistema $ \bm{x}'(t)=A\,\bm{x}(t) $, con $ A \in \R^{n,n} $. Siano \begin{itemize}
        \item $ \{\lambda_1,\dots, \lambda_{h} \} $ autovalori reali, di molteplicità algebrica, rispettivamente $ m_1,\dots,m_{h}  $; 
        \item $ \{\mu_1, \overline{\mu}_1,\dots,\mu_{k},\overline{\mu}_k \} $ autovalori complessi, $ \mu_{j}, \overline{\mu}_j=a_{j} \pm b_{j}\,i    $, di molteplicità algebrica, rispettivamente $ n_1,\dots,n_{k}  $
